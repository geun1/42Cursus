...
/*   Updated: 2023/01/07 19:28:21 by gsong            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "checker.h"

static char	*gnl_strdup(const char *s1, int size)
{
	char	*cp;
	int		i;
...
static char	*read_line(char **backup, char *buf)
{
	...
	{
		if (*backup[0] != '\0')
			ret = gnl_strdup(*backup, gnl_strlen(*backup));
		free(*backup);
		*backup = NULL;
	}
	else
	{
		ret = gnl_strdup(*backup, gnl_strchr(*backup) + 1);
		n_b_s = gnl_strlen(*backup + gnl_strchr(*backup) + 1);
		new_backup = gnl_strdup((*backup + gnl_strchr(*backup) + 1), n_b_s);
		free(*backup);
		*backup = new_backup;
	}
	free(buf);
