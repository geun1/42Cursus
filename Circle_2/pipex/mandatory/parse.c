...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/01 20:21:17 by gsong             #+#    #+#             */
/*   Updated: 2023/09/05 02:15:41 by gsong            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	quotes(char **av, int i, t_info *info)
{
	...
	while (
		...
		ft_strncmp(info->i_cmd[i].cmd[j], "\"", 1) != 0 && info->i_cmd[i].cmd[j])
		j++;
	tmp = j;
	...
	if (!ft_strncmp(info->i_cmd[i].cmd[tmp], "\'", 1))
